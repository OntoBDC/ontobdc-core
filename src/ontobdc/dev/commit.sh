    ...
    (
        ...
        if ! git diff-index --quiet HEAD --; then
            ...
            if git remote | grep -q "."; then
                ...
                SSH_KEY_PATH=$(PYTHONPATH="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd):${PYTHONPATH:-}" python3 -c "import sys; from ontobdc.dev import get_ssh_key_path; sys.stdout.write(get_ssh_key_path(sys.argv[1]))" "$ROOT_DIR" 2>/dev/null)
