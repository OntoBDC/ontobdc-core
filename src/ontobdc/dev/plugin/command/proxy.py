...
class DevProxyCommand(CliCommandPort):
    ...
    METADATA: CliCommandMetadata = CliCommandMetadata(
        ...
        description="Delegate dev commands to the ontobdc-dev workspace CLI.",
        arguments=[
            {
                "accepts": ["dev"],
    ...
    def accepts(args: List[str]) -> bool:
        return len(args) > 0 and args[0] == "dev"
    ...
    def run(self) -> CommandResponse:
        ...
        if dev_src_path is None:
            ...
            raise ModuleNotFoundError(
                "Unable to locate the ontobdc-dev package. Install "
                "ontobdc-dev in the active Python environment or place its "
    ...
    def _resolve_installed_dev_src_path(self) -> Optional[Path]:
        try:
            module_spec: Optional[ModuleSpec] = find_spec("ontobdc_dev")
    ...
    def _is_valid_dev_package_path(self, package_path: Path) -> bool:
        return (
            package_path.is_dir()
            and package_path.name == "ontobdc_dev"
