...
class ContextHelpCommand(CliCommandPort):
    """
    Command for displaying help information for the context component.
    """

    METADATA = CliCommandMetadata(
        id="help",
        logical_component="context",
        description="Display help information for the context component.",
        arguments=[
            {
                "accepts": [
                    "--help",
    ...
    def accepts(args: List[str]) -> bool:
        """
        Match the context help command at the CLI routing stage.
        """
        return len(args) > 1 and args[0] == "context" and args[1] in ["--help", "-h"]
    ...
    def run(self) -> CommandResponse:
        ...
        usage_list: Dict[str, str] = {
            "base": "ontobdc context <argument> [flags/parameters]"
        }
        loader: CommandLoader = CommandLoader("context", self._logger)
        for command in loader.get_all():
            if (
                command.METADATA.id != "base"
                and hasattr(command.METADATA, "arguments")
                and command.METADATA.arguments
            ):
                arg_key: str = " | ".join(
                    command.METADATA.arguments[0]["accepts"]
                )
                arg_list[arg_key] = command.METADATA.arguments[0]["description"]
                if "usage" in command.METADATA.arguments[0]:
                    usage_list[command.METADATA.id] = command.METADATA.arguments[0][
                        "usage"
                    ]

        arg_list[
            " | ".join(self.METADATA.arguments[0]["accepts"])
        ] = self.METADATA.arguments[0]["description"]

        return HelpCommandResponse(
            title="Context CLI Help",
            description="Display help information for the context component.",
            content={
                "Usage": usage_list,
                "Options": arg_list,
