...
class EntityVectorRepositoryAdapter:
    ...
    REMOTE_ONTOLOGY_REF: str = "54de39c45b10b6a991d227d76eb4fa6fccbb12ee"
    ...
    def sync_remote_ontology_vector_cache(self) -> Dict[str, Any]:
        ...
        remote_paths: List[str] = self._list_remote_ontology_vector_paths()
        for remote_path in remote_paths:
            target_path: Path = self.cache_root / Path(remote_path)
            target_path.parent.mkdir(parents=True, exist_ok=True)
            target_path.write_text(self._download_remote_text_file(remote_path), encoding="utf-8")
            cache_files.append(str(target_path))

        return {
            "remote_ontology_ref": self.REMOTE_ONTOLOGY_REF,
            "remote_vector_path_count": len(remote_paths),
            "cached_vector_files": cache_files,
            "cache_root": str(self.cache_root),
        }

    def load_registered_candidates(self, file_paths: List[str]) -> Dict[str, Any]:
        ontology_payload: Dict[str, Any] = self.load_candidates(file_paths=file_paths, source_kind="ontology")
    ...
    def load_candidates(self, file_paths: List[str], source_kind: str) -> Dict[str, Any]:
        ...
        for raw_file_path in file_paths:
            file_path: Path = Path(raw_file_path).expanduser().resolve()
            ...
            for subject in set(graph.subjects()):
                ...
                identifier_value: Optional[str] = str(identifier_literal) if identifier_literal is not None else None
                ...
                candidates.append(
                    {
                        ...
                        "supported_file_type": (
                            str(supported_file_type_literal) if supported_file_type_literal is not None else ""
                        ),
        ...
        deduplicated_candidates: List[Dict[str, Any]] = self._deduplicate_candidates(candidates)
    ...
    def load_learning_record_candidates(self) -> Dict[str, Any]:
        context_file_path: Path = self._project_root / ".__ontobdc__" / "context.ttl"
    ...
    def _list_remote_ontology_vector_paths(self) -> List[str]:
        ...
        for entry in list(payload.get("tree", [])):
            ...
            if not path.startswith("ontology/"):
                continue
            if not path.endswith("/vector.ttl") and path != "ontology/vector.ttl":
                continue
    ...
    def _download_remote_text_file(self, remote_path: str) -> str:
        request = Request(
            self._remote_raw_file_url(remote_path),
            headers={"User-Agent": "OntoBDC"},
        )
        with urlopen(request, timeout=30) as response:
    ...
    def _remote_tree_api_url(self) -> str:
        return (
            ...
            f"{self.REMOTE_OWNER}/{self.REMOTE_REPOSITORY}/git/trees/{self.REMOTE_ONTOLOGY_REF}?recursive=1"
        )

    def _remote_raw_file_url(self, remote_path: str) -> str:
        return (
            "https://raw.githubusercontent.com/"
            f"{self.REMOTE_OWNER}/{self.REMOTE_REPOSITORY}/{self.REMOTE_ONTOLOGY_REF}/{remote_path}"
        )
    ...
    def _first_literal_by_local_name(
        ...
        for _, predicate, obj in graph.triples((subject, None, None)):
            if self._local_name(predicate) != local_name:
                continue
            if isinstance(obj, Literal):
                return obj
        return None

    def _first_object_by_local_name(
        self,
        graph: Graph,
        subject: URIRef,
        local_name: str,
    ...
    def _parse_vector_literal(self, literal: Literal) -> List[float]:
        payload: Any = json.loads(str(literal).strip())
    ...
    def _deduplicate_candidates(self, candidates: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for candidate in candidates:
            candidate_uri: str = str(candidate["candidate_uri"]).strip()
        ...
        return sorted(
            ...
            key=lambda candidate: (str(candidate.get("candidate_uri", "")), str(candidate.get("source_file", ""))),
        )

    def _should_replace_candidate(self, current_candidate: Dict[str, Any], new_candidate: Dict[str, Any]) -> bool:
        current_is_cached: bool = self._is_cached_source_file(str(current_candidate.get("source_file", "")))
        new_is_cached: bool = self._is_cached_source_file(str(new_candidate.get("source_file", "")))
