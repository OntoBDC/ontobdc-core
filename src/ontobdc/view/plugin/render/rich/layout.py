...
class RichMessageBoxLayout(MessageBoxLayoutRendererPort):
    """
    Rich-compatible message box layout that reproduces the CLI shell box.
    """
    ...
    ANSI_ESCAPE_PATTERN: ClassVar[Pattern[str]] = re.compile(r"\x1b\[[0-9;]*m")
    MARKDOWN_PATTERN: ClassVar[Pattern[str]] = re.compile(
        r"(^\s{0,3}#{1,6}\s)|(^\s*[-*+]\s)|(\*\*[^*]+\*\*)|(^```)|(`[^`]+`)",
        re.MULTILINE,
    )
    HEADING_PATTERN: ClassVar[Pattern[str]] = re.compile(r"^\s{0,3}(#{1,6})\s+(.*)$")
    BULLET_PATTERN: ClassVar[Pattern[str]] = re.compile(r"^(\s*)[-*+]\s+(.*)$")
    ...
    def render(self, layout_data: MessageBoxLayoutData) -> str:
        ...
        title_type = layout_data.title_type
        title_text = layout_data.title

        # layout_data.partial
        message_text = layout_data.content
        subtitle_text = layout_data.subtitle
        footer_text = layout_data.footer

        term_cols: int = self._resolve_terminal_width()
        inner_width: int = term_cols - 2
        ...
        if subtitle_text:
            subtitle_line: str = f"{self.GRAY}{subtitle_text}{self.RESET}"
            lines.append(self._frame_line(subtitle_line, inner_width, border_color))
            lines.append(self._empty_line(inner_width, border_color))

        lines.extend(self._build_message_lines(message_text, inner_width, border_color))

        if footer_text:
            lines.append(self._empty_line(inner_width, border_color))
            lines.append(self._build_footer_line(footer_text, inner_width, border_color))
        else:
            lines.append(f"{border_color}╰{horizontal_line}╯{self.RESET}")

        return "\n".join(lines)

    def _build_title_content(self, color: str, title_type: str, title_text: str) -> str:
        title_type_color: str = self.BLUE if title_type == "OntoBDC" else self._resolve_color(color)
        title_head: str = f" >_ {self.BOLD}{title_type_color}{title_type}{self.RESET}"
    ...
    def _build_message_lines(self, message_text: str, inner_width: int, border_color: str) -> List[str]:
        raw_lines: List[str] = self._resolve_message_lines(message_text, inner_width) or [""]
        framed_lines: List[str] = []

        for raw_line in raw_lines:
            wrapped_chunks: List[str] = self._wrap_visible_line(raw_line, inner_width)

            for chunk in wrapped_chunks:
                gray_chunk: str = f"{self.GRAY} {chunk}{self.RESET}"
                framed_lines.append(self._frame_line(gray_chunk, inner_width, border_color))

        return framed_lines

    def _resolve_message_lines(self, message_text: str, inner_width: int) -> List[str]:
        _ = inner_width
    ...
    def _render_markdown_lines(self, message_text: str) -> List[str]:
        ...
        for raw_line in message_text.splitlines():
            ...
            if heading_match is not None:
                ...
                heading_text: str = self._strip_inline_markdown(heading_match.group(2).strip())
                lines.extend(self._format_heading_line(heading_text, heading_level))
                continue
    ...
    def _format_heading_line(self, heading_text: str, heading_level: int) -> List[str]:
        if heading_level <= 2:
            return [heading_text, ""]
    ...
    def _strip_inline_markdown(self, content: str) -> str:
        ...
        normalized_content = self.INLINE_CODE_PATTERN.sub(r"\1", normalized_content)
    ...
    def _build_footer_line(self, footer_text: str, inner_width: int, border_color: str) -> str:
        ...
        return f"{border_color}╰{left_line} {footer_text} {right_line}╯{self.RESET}"

    def _frame_line(self, content: str, inner_width: int, border_color: str) -> str:
        padded_content: str = self._pad_visible_content(content, inner_width)
        return f"{border_color}│{self.RESET}{padded_content}{border_color}│{self.RESET}"

    def _empty_line(self, inner_width: int, border_color: str) -> str:
        empty_content: str = " " * inner_width
        return f"{border_color}│{self.RESET}{empty_content}{border_color}│{self.RESET}"

    def _pad_visible_content(self, content: str, inner_width: int) -> str:
    ...
    def _wrap_visible_line(self, content: str, inner_width: int) -> List[str]:
    ...
    def _resolve_terminal_width(self) -> int:
        terminal_size = shutil.get_terminal_size(fallback=(self.DEFAULT_TERMINAL_WIDTH, 24))
    ...
    def _resolve_color(self, color: str) -> str:
        resolved_color: str = self.COLOR_MAP.get(color.upper(), color)
    ...
    def _visible_length(self, content: str) -> int:
        visible_content: str = self._strip_ansi(content)
        return len(visible_content)
