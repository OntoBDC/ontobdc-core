...
def _patched_bootstrap(content: str) -> str:
    if _BOOTSTRAP_LEGACY_REPLACEMENT in content:
        return content.replace(
            _BOOTSTRAP_LEGACY_REPLACEMENT,
            _BOOTSTRAP_REPLACEMENT,
            1,
        )
    if _BOOTSTRAP_REPLACEMENT in content:
        return content
    if _BOOTSTRAP_ORIGINAL not in content:
        raise ValueError(
...
def materialize_workstream_schedule_relations(
    ...
    """Publish relation data and one integrated board runtime for the view."""
