...
class IsPublishableCapability(TransactionCapability):
    METADATA = CapabilityMetadata(
        ...
        tags=["view", "container", "publication", "transformation"],
        supported_languages=["en", "pt-br"],
    )

    def label(self, lang: str = "en") -> str:
        return ContainerViewProcessState.IS_PUBLISHABLE.label(lang)

    def description(self, lang: str = "en") -> str:
        return ContainerViewProcessState.IS_PUBLISHABLE.description(lang)

    def execute(self, context: CliContextPort) -> Dict[str, Any]:
        result = ensure_publishable(context)
        result["resulting_state"] = ContainerViewProcessState.IS_PUBLISHABLE
        return result
