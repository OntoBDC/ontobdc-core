...
def _has_only_expected_triples(
    storage_graph: Graph,
    subject: URIRef,
    ...
    found_types: List[object] = list(storage_graph.objects(subject, RDF.type))
    if found_types != [OBDC.DataContainer]:
        return False

    for predicate, expected_obj in expected_triples.items():
        actual_values: List[object] = list(storage_graph.objects(subject, predicate))
        if actual_values != [expected_obj]:
            return False

    for predicate, obj in storage_graph.predicate_objects(subject):
        if predicate not in allowed_predicates:
            return False
        if predicate == RDF.type and obj != OBDC.DataContainer:
            return False
...
def main(
    ...
    identifier_subjects: List[URIRef] = [
        subject
        for subject in storage_graph.subjects(DCTERMS.identifier, identifier_obj)
        if isinstance(subject, URIRef)
    ]
    for subject in identifier_subjects:
        if subject != container_subject:
            return 1

    if not _has_only_expected_triples(storage_graph, container_subject, expected_triples):
        return 1
