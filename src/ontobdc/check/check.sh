...
CONFIG_JSON="${ONTOBDC_DIR}/check/config.json"
CONFIG_YAML="${MODULE_ROOT}/.__ontobdc__/config.yaml"
...
beh=(base.get('behavior') or {}) if isinstance(base,dict) else {}
...
run_checks() {
    ...
    CHECKS_DICT=$(python3 "${ONTOBDC_DIR}/check/adapter/run.py" "$CONFIG_JSON" "$CONFIG_YAML" "$NAME" "$ENGINE" 2>/dev/null)
...
try:
    import yaml
    with open('$CONFIG_YAML') as f: yaml_data = yaml.safe_load(f) or {}
...
for scope_name in $SCOPES; do
    ...
    if [[ ! -d "${CHECK_DIR}" ]]; then
        for candidate in \
            "$MODULE_ROOT/wip/src/ontobdc/${scope_name}/check" \
