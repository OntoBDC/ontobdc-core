...
from ontobdc.core.domain.port.storage import EntityStoragePort
from rdflib import Graph, Namespace, URIRef, Literal, RDF, OWL, XSD


class EntityStorageAdapter(EntityStoragePort):
    ...
    def get(unique_name: str) -> Entity:
        if not EntityStorageAdapter.exists(unique_name):
            raise ValueError(f"Entity {unique_name} does not exist")

        return Entity(EntityStorageAdapter.load(unique_name))

    @staticmethod
    def load(unique_name: str) -> Dict[str, Any]:
        return json.loads(Path(EntityStorageAdapter.entity_path(unique_name)).read_text(encoding="utf-8"))
