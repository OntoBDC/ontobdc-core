...
if [ $? -ne 0 ]; then
    echo -e "${RED}✗ Missing build dependency: setuptools${RESET}"
    echo -e "${YELLOW}This environment does not have setuptools installed, and pip needs it to install this project.${RESET}"
    echo -e "${YELLOW}If you are offline, pip cannot download build dependencies.${RESET}"
    echo -e "${YELLOW}Fix:${RESET} pip install \"setuptools>=61\""
    exit 1
fi
